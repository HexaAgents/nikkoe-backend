...
class ItemRepository:
    def find_all(self, limit: int = 50, offset: int = 0) -> dict:
        ...
        categories_map = batch_load(
            "categories", "category_id", list({i["category_id"] for i in items if i.get("category_id")})
        )

        balances = _safe_query_view(supabase.table("inventory_balances").select("*").in_("item_id", item_ids))

        rl_resp = supabase.table("receipt_lines").select("*").in_("item_id", item_ids).execute()
    ...
    def find_by_id(self, id: str) -> dict | None:
        response = supabase.table("items").select("*, categories(name)").eq("item_id", id).maybe_single().execute()
        return response.data

    def create(self, data: dict) -> dict:
        response = supabase.table("items").insert(data).select().single().execute()
        return response.data

    def update(self, id: str, data: dict) -> dict:
        response = supabase.table("items").update(data).eq("item_id", id).select().single().execute()
        return response.data
