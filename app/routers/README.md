...
- **auth.py** -- Auth router at `/api/auth` with four endpoints: `POST /login` (authenticates via Supabase, returns JWT), `POST /signup` (creates account), `POST /refresh` (exchanges a refresh token for new access/refresh tokens), `GET /me` (returns current user profile), and `POST /change-password` (requires JWT, verifies current password then updates). Login, signup, and refresh are public; me and change-password require authentication.
...
- **inventory.py** -- Inventory router at `/api/inventory` with five endpoints: `GET /movements` (paginated with item/user relations), `GET /on-hand` (all positive balances), `GET /stock-valuation` (full valuation report), `POST /transfer` (same-item location transfer), and `POST /transfer-cross` (cross-item transfer between different part numbers/locations). Transfer endpoints inject the authenticated user's ID.
- **items.py** -- Items router at `/api/items` with eleven endpoints: CRUD (`GET /`, `GET /{id}`, `POST /`, `PUT /{id}`, `DELETE /{id}`), search (`GET /search` with `sort_by` parameter for sorting by receipt date, sale date, or quantity), `GET /by-search-id/{search_id}` (find items sharing the same normalized search ID), plus four sub-resource queries (`GET /{id}/quotes`, `GET /{id}/inventory`, `GET /{id}/receipts`, `GET /{id}/sales`). Instantiates 5 repositories at module level to serve the item detail page.
