...
- **dependencies.py** -- Creates the singleton Supabase client using the service role key with a custom `httpx.Client` configured for HTTP/1.1 and a 20-connection pool, avoiding HTTP/2 multiplexing issues under concurrent request bursts. This single instance is imported by every repository. A separate `supabase_auth` client is used for auth operations.
