...
- **inventory.py** -- `InventoryService` with `list_movements` and `list_on_hand`. Both delegate directly to the inventory repository with no additional business logic.
- **item.py** -- `ItemService`, the most complex service, whose constructor takes 5 repositories (item, supplier_quote, inventory, receipt, sale) because the item detail page needs data from 5 different tables. Methods: `list_items`, `get_item` (throws `NotFoundError` if null), `get_item_quotes`, `get_item_inventory`, `get_item_receipts`, `get_item_sales`, `create_item`, `update_item`, and `delete_item`.
